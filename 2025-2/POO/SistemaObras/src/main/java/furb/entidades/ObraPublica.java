...
public class ObraPublica extends Obra {
    ...
    // 🔹 Construtor padrão
    // 🔹 Construtor completo
    public ObraPublica(int p_seqobr, int p_codobr, String p_desobr, LocalDate p_datetr,
                       TipoObra p_tipobr, List<Profissional> p_proobr,
                       LocalDate p_datini, LocalDate p_datfim) {
    ...
    public String toString() {
        return String.format(
            "Obra Pública [%d - %s]: %s (Duração: %d dias)",
            codobr, desobr,
            datini != null && datfim != null
                ? String.format("%s até %s", datini, datfim)
